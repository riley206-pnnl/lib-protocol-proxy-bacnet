...
logging.basicConfig(filename='protoproxy.log',
                    level=logging.DEBUG,
...
class BACnetManager:
    ...
    def run(self):
        self.ppm.start()
        self.ppm.get_proxy((self.local_device_address, 0),
    ...
    def main_loop(self):
        while not self.ppm._stop:
            ...
            result = self.ppm.send(
                self.ppm.peers[proxy_id],
                ProtocolProxyMessage(method_name='QUERY_DEVICE',
                                     payload=json.dumps({
                                         'address': '130.20.24.157'
                                     }).encode('utf8'),
            ...
            if isinstance(result, AsyncResult):
                ...
                _log.debug(
                    len([
                        ObjectIdentifier(tuple(r)) for r in json.loads(result.get().decode('utf8'))
                    ]))
                _log.debug('\n\n')
